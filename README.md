...
通过现有 SSH 配置监控 Linux 服务器的 macOS 菜单栏应用。状态栏只显示一个服务器图标；点击后可查看 SSH 状态、CPU、系统内存已用/总量，以及每张 NVIDIA GPU 的利用率、显存已用/总量和温度。菜单栏、弹窗标题和悬停提示不展示主机别名。没有 NVIDIA GPU 时，CPU、内存和 SSH 状态仍可使用。
...
3. 点击菜单栏中的服务器图标，在「设置」中填写自己的 SSH 别名并保存。默认每 2 秒采集一次；一次采集尚未结束时会跳过下一次，避免连接重叠。
